...
function statusTag(s){let c=s==='Won'||s==='Qualified'?'good':s==='Lost'?'bad':s==='Meeting'?'warn':'';return `<span class="tag ${c}">${escapeHtml(s)}</span>`}
...
async function render(){
  ...
  const stages=['New','Contacted','Qualified','Meeting','Won','Lost'];document.getElementById('pipeline').innerHTML=stages.map(s=>`<div class="pipe"><span>${s}</span><strong>${scoped.filter(x=>x.status===s).length}</strong></div>`).join('');
  const todays=scoped.filter(x=>x.followup_date===today()&&!['Won','Lost'].includes(x.status));document.getElementById('todayFollowups').innerHTML=todays.length?todays.slice(0,5).map(x=>`<div class="list-item"><div><strong>${escapeHtml(x.name)}</strong><div class="meta">${escapeHtml(x.client_name||'')} · ${escapeHtml(x.phone)}</div></div>${statusTag(x.status)}</div>`).join(''):'<div class="muted">No follow-ups due today.</div>';
